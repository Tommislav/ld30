...
import com.haxepunk.masks.CloudGrid;
...
class TmxEntity extends Entity
{
	...
	public function loadGraphic(tileset:String, layerNames:Array<String>, skip:Array<Int> = null)
	{
		...
		for (name in layerNames)
		{
			...
			var scrollx:Float = layer.properties.has("scrollx") ? Std.parseFloat(layer.properties.resolve("scrollx")) : 1;
			var scrolly:Float = layer.properties.has("scrolly") ? Std.parseFloat(layer.properties.resolve("scrolly")) : 1;
	...
	public function loadMask(collideLayer:String = "collide", typeName:String = "solid", skip:Array<Int> = null)
	{
		...
		var grid = new CloudGrid(map.fullWidth, map.fullHeight, map.tileWidth, map.tileHeight);
		...
		for (row in 0...layer.height)
		{
			for (col in 0...layer.width)
			{
				originalGid = layer.tileGIDs[row][col];
				gid = originalGid - 1; // why -1??
				
				if (gid < 0) continue;
				if (skip == null || Lambda.has(skip, gid) == false)
				{
					
					var tileType:String = map.getGidProperty(originalGid, "type");
					var collisionFlag:Int = (tileType == "cloud") ? CloudGrid.CLOUD : CloudGrid.SOLID;
					
					grid.setTile(col, row, collisionFlag);
